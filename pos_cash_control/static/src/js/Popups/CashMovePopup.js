odoo.define("pos_cash_control.CashMovePopup", function (require) {
    "use strict";
    ...
    class CashMovePopup extends AbstractAwaitablePopup {
        ...
        confirm() {
            ...
            if (this.state.inputType === "out" && this.state.inputAmount > 0) {
                this.state.inputHasError = true;
                this.errorMessage = this.env._t(
                    "Insert a negative amount with the Cash Out option."
                );
                return;
            }
            if (this.state.inputType === "in" && this.state.inputAmount < 0) {
                this.state.inputHasError = true;
                this.errorMessage = this.env._t(
                    "Insert a positive amount with the Cash In option."
                );
                return;
            }
            ...
            if (event.key === "-") {
                ...
                this.state.inputAmount =
                    this.state.inputType === "out"
                        ? this.state.inputAmount.substring(1)
                        : `-${this.state.inputAmount}`;
    ...
    CashMovePopup.template = "CashMovePopup";
