...
export class EventSelectorPopup extends Component {
    ...
    getEventDates(event) {
        ...
        if (event.date_begin) {
            try {
                const startDate = DateTime.fromISO(event.date_begin)
                    .startOf("day")
                    .toJSDate();
     ...
     */
    _applyFilters(event) {
        return this.dialogState.filters.every((filter) =>
             this._applyFilter(event, filter));
    }
    ...
    _applyFilter(event, filter) {
        if (filter.kind === "product") {
            const ticketsWithProduct = this.pos.models["event.event.ticket"]
                .getAll()
                .filter(
                    (ticket) => 
