    ...
    def write(self, vals):
        for rec in self:
            if "available_pricelist_ids" in vals:
                if "selectable_pricelist_ids" in vals and vals["selectable_pricelist_ids"] and vals["selectable_pricelist_ids"][0][0] == 6:
                    selectable_ids_current_or_new = set(vals["selectable_pricelist_ids"][0][2])
                else:
                    selectable_ids_current_or_new = set(rec.selectable_pricelist_ids.ids)

                available_ids_from_vals = set()
                if "available_pricelist_ids" in vals and vals["available_pricelist_ids"] and vals["available_pricelist_ids"][0] and vals["available_pricelist_ids"][0][0] == 6:
                    available_ids_from_vals = set(vals["available_pricelist_ids"][0][2])
                
                intersection = list(selectable_ids_current_or_new.intersection(available_ids_from_vals))
