...
patch(PosStore.prototype, {
    async _processData(loadedData) {
        ...
        if (!this.config || !this.pricelists) {
            console.error("POS Custom Pricelist: 'this.config' or 'this.pricelists' are not available after _processData. Skipping custom logic.");
            return;
        }
        ...
        const setPricelistMethod = typeof this.set_pricelist === 'function' ? this.set_pricelist.bind(this) : null;
        const posService = this.env && this.env.services && this.env.services.pos;
        const setPricelistViaService = posService && typeof posService.set_pricelist === 'function' ? posService.set_pricelist.bind(posService) : null;
        ...
        if (initialConfigPricelist) {
            if (setPricelistMethod) {
                ...
                console.warn("POS Custom Pricelist: set_pricelist method not found via 'this' or 'pos' service for initial config pricelist.");
            }
            ...
            if (setPricelistMethod) {
                ...
                console.warn("POS Custom Pricelist: set_pricelist method not found via 'this' or 'pos' service for default pricelist.");
            }
            ...
            if (setPricelistMethod) {
                ...
                console.warn("POS Custom Pricelist: set_pricelist method not found via 'this' or 'pos' service for first available pricelist.");
            }
        } else {
            if (setPricelistMethod) {
                ...
                console.warn("POS Custom Pricelist: set_pricelist method not found via 'this' or 'pos' service for null pricelist.");
            }
        }

        console.log("POS Custom Pricelist: Logic applied successfully via _processData.");
    },
});
