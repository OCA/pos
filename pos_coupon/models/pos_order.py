...
class PosOrder(models.Model):
    ...
    used_coupon_ids = fields.One2many(
        "sale.coupon", "pos_order_id", string="Consumed Coupons"
    )
    generated_coupon_ids = fields.One2many(
        "sale.coupon", "source_pos_order_id", string="Generated Coupons"
    )
    ...
    def validate_coupon_programs(
        ...
        self.env["sale.coupon"].browse(unused_coupon_ids).write({"state": "new"})
        self.write(
            {
...
class PosOrderLine(models.Model):
    ...
    coupon_id = fields.Many2one(
        "sale.coupon", string="Coupon", help="Coupon that generated this reward.",
    )
