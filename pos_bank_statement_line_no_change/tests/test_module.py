...
class TestModule(TransactionCase):
    ...
    def test_write_unlink(self):
        order = self._create_order()

        # Trying to write on payment lines of paid orders should fail
        with self.assertRaises(UserError):
            order.mapped("statement_ids").write({"amount": 9999.9})

        # Trying to unlink payment lines of paid orders should fail
        with self.assertRaises(UserError):
            order.mapped("statement_ids").unlink()
