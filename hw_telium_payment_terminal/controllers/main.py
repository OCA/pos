...
class TeliumPaymentTerminalDriver(Thread):
    def __init__(self):
        ...
        self.device_name = config.get("telium_terminal_device_name", "/dev/ttyACM0")
    ...
    def get_answer_from_terminal(self, data):
        ...
        if msg[0] != chr(ascii_names.index("STX")):
            logger.error("The first byte of the answer from terminal should be STX")
        ...
        if computed_lrc != lrc:
            logger.error("The LRC of the answer from terminal is wrong")
        real_msg = msg[1:-2]
...
class TeliumPaymentTerminalProxy(hw_proxy.Proxy):
    ...
    def payment_terminal_transaction_start(self, payment_info):
        logger.debug(
            "Telium: Call payment_terminal_transaction_start with " "payment_info=%s",
            payment_info,
        )
