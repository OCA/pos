...
class WizardUpdateBankStatement(models.TransientModel):
    ...
    name = fields.Char(related="statement_id.name")
    journal_id = fields.Many2one(
        comodel_name="account.journal",
        related="statement_id.journal_id",
    )
    balance_start = fields.Monetary(
        string="Starting Balance", default=0.0, compute="_compute_balance_start"
    )
    ...
    balance_end = fields.Monetary(
        string="Computed Balance", default=0.0, compute="_compute_balance_end"
    )
    ...
    def default_get(self, flds):
        ...
        if not active_pos_id:
            return res
        assert active_model == "pos.session", "Bad context propagation"
        if len(active_pos_id) > 1:
            raise UserError(
                _("You cannot start the closing " "balance for multiple POS sessions")
            )
