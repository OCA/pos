...
odoo.define("pos_payment_credit.screens", function (require) {
    ...
    PaymentScreenWidget.include({
        customer_changed: function () {
            ...
            if (client && client.credit_amount) {
                this.$(".js_customer_credit").text(
                    _t("Available Credit:") +
                        " " +
                        this.format_currency(client.credit_amount)
                );
            } else {
                this.$(".js_customer_credit").text("");
        ...
        apply_credit_amount: function () {
            ...
            if (line && line.get_credit_payment()) {
                ...
                if (this.pos.config.auto_apply_credit_amount) {
                    this.$(".js_customer_credit_payment").text(
                        _t("A refund of ") + amountFormatted + _t(" will be applied")
                    );
                } else {
                    self.gui.show_popup("confirm", {
                        ...
                        cancel: function () {
                            ...
                            if (selectedPaymentLineEle) {
                                var paymentLineId = $(selectedPaymentLineEle[0])
                                    .find(".delete-button")
                                    .data("cid");
                                if (paymentLineId) {
        ...
        render_paymentline_credit_amount: function () {
            ...
            if (lines.length === 0 && this.pos.config.auto_apply_credit_amount) {
                ...
                if (
                    ...
                    client.credit_amount.toFixed(4) > 0
                ) {
                    var cashregister = _.find(
