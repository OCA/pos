    ...
    ***********************************************************/
    var SelectVariantPopupWidget = popups.extend({
        ...
        start: function () {
            ...
            this.attribute_list_widget.replace(
                this.$(".placeholder-AttributeListWidget")
            );

            // Add behaviour on Cancel Button
            this.$("#variant-popup-cancel")
                .off("click")
                .click(function () {
                    self.hide();
                });
        ...
        show: function (product_tmpl_id) {
            ...
            // Render Variants
            var variant_ids = this.pos.db.template_by_id[product_tmpl_id]
                .product_variant_ids;
        ...
        filter_variant: function () {
            ...
            for (var index in this.variant_list) {
                ...
                for (var i = 0; i < value_list.length; i++) {
                    found =
                        found &&
                        variant.attribute_value_ids.indexOf(value_list[i]) != -1;
                }
                if (found) {
    ...
    var AttributeListWidget = PosBaseWidget.extend({
        ...
        init: function (parent, options) {
            ...
            this.click_set_attribute_handler = function (event) {
                ...
                for (
                    var i = 0;
                    i < parent.children[1].children[0].children.length;
                    i++
                ) {
                ...
                self.__parentedParent.variant_list_widget.set_filter(
                    this.dataset.attributeId,
                    this.dataset.attributeValueId
                );
            };
            this.click_reset_attribute_handler = function (event) {
                ...
                self.__parentedParent.variant_list_widget.reset_filter(
                    this.dataset.attributeId
                );
            };
            this._super(parent, options);
        ...
        render_attribute: function (attribute) {
            ...
            for (var i = 0, len = attribute.value_ids.length; i < len; i++) {
                var value = this.pos.db.get_product_attribute_value_by_id(
                    attribute.value_ids[i]
                );
                var product_list = this.pos.db.get_product_by_ids(
                    this.product_template.product_variant_ids
                );
                var subproduct_list = this.pos.db.get_product_by_value_and_products(
                    value.id,
                    product_list
                );
                ...
                if (variant_qty != 0) {
                    ...
                    value_node.addEventListener(
                        "click",
                        this.click_set_attribute_handler
                    );
        ...
        renderElement: function () {
            ...
            for (var i = 0, len = this.attribute_list.length; i < len; i++) {
                ...
                attribute_node
                    ...
                    .addEventListener("click", this.click_reset_attribute_handler);
    ...
    PosDB.include({
        ...
        add_templates: function (templates) {
            for (var i = 0; i < templates.length; i++) {
                ...
                for (var j = 0; j < templates[i].product_variant_ids.length; j++) {
                    var product = this.product_by_id[
                        templates[i].product_variant_ids[j]
                    ];
                    for (var k = 0; k < product.attribute_value_ids.length; k++) {
                        if (
                            attribute_value_ids.indexOf(
                                product.attribute_value_ids[k]
                            ) == -1
                        ) {
        ...
        add_product_attributes: function (product_attributes) {
            for (var i = 0; i < product_attributes.length; i++) {
                ...
                this.product_attribute_by_id[product_attributes[i].id] =
        ...
        add_product_attribute_values: function (product_attribute_values) {
            for (var i = 0; i < product_attribute_values.length; i++) {
                ...
                this.product_attribute_value_by_id[product_attribute_values[i].id] =
                    product_attribute_values[i];
            }
        },
    });
