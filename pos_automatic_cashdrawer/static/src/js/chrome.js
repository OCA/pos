        ...
        */
        init: function () {
            ...
            this.ready.done(function () {
                if (
                    self.pos.config.iface_automatic_cashdrawer &&
                    self.pos.config.cash_control
                ) {
                        ...
                        $.when(
                                ...
                                if (totals.total != balance_start) {
                                    ...
                                    if (
                                        user.groups_id.indexOf(
                                            self.pos.config
                                                .group_pos_automatic_cashlogy_config[0]
                                        ) != -1
                                    ) {
                                        self.gui.show_popup("confirm", {
                                            ...
                                            body:
                                                _t(
                                                    "The opening balance for this session does not match with the Cashdrawer Inventory."
                                                ) +
                                                "\n" +
                                                _t(
                                                    "Do you want to overwrite it with the real amount?"
                                                ) +
                                                "\n\n" +
                                                _t("Cashdrawer Inventory: ") +
                                                self.format_currency(totals.total) +
                                                "\n" +
                                                _t("Session Starting Balance: ") +
                                                self.format_currency(balance_start),
                                            confirm: function () {
                                                framework.blockUI();
                                                self.pos
                                                    .action_set_balance(
                                        ...
                                        self.gui.show_popup("error", {
                                            ...
                                            body:
                                                ...
                                                "\n" +
                                                _t(
                                                    "Please ask your manager to fix it."
                                                ),
                                        });
                                    }
                                    ...
                                    framework.blockUI();
                                    self.pos
                                        .action_set_balance(inventory.total, "start")
                                        .always(function () {
                                            framework.unblockUI();
                                        });
                                }
                            ...
                            .fail(function (error) {
                                self.gui.show_popup("error", {
                                    ...
                                    body:
                                        _t(
                                            "Check that the Cashdrawer is online before starting the session, and refresh the browser.\n\n"
                                        ) + error.data.message,
                                });
                            })
                            .always(function () {
        ...
        */
        close: function () {
            ...
            self.chrome.loading_message(
                _t("Synchronizing automatic cashdrawer inventory")
            );
            self.pos.proxy
                .automatic_cashdrawer_get_inventory()
                .then(function (inventory) {
                    self.pos
                        .action_set_balance(inventory.total, "end")
                        .then(function () {
