...
odoo.define("pos_crm.NumberBuffer", function (require) {
    ...
    const _updateBuffer = (input) => {
        let self = NumberBuffer;
        const isEmpty = val => {
            return val === '' || val === null;
        };
        if (input === undefined || input === null) return;
        let isFirstInput = isEmpty(self.state.buffer);
        if (input === ',' || input === '.') {
            if (isFirstInput) {
                ...
                self.state.buffer = self.state.buffer + self.decimalPoint;
            }
            ...
            if (isEmpty(buffer)) {
                ...
                const nCharToRemove = buffer[buffer.length - 1] === self.decimalPoint ? 2 : 1;
                self.state.buffer = buffer.substring(0, buffer.length - nCharToRemove);
            }
        } else if (input === '+') {
            if (self.state.buffer[0] === '-') {
                self.state.buffer = self.state.buffer.substring(1, self.state.buffer.length);
            }
        } else if (input === '-') {
            if (isFirstInput) {
                self.state.buffer = '-0';
            ...
            const currentBufferValue = self.state.buffer ? parse.float(self.state.buffer) : 0;
