odoo.define("pos_kiosk.ChromeKiosk", function (require) {
    "use strict";
    ...
    class ChromeKiosk extends PopupControllerMixin(PosComponent) {
        constructor() {
            super(...arguments);
        ...
        async start() {
            try {
                const posModelDefaultAttributes = {
                    ...
                    do_action: this.props.webClient.do_action.bind(
                        this.props.webClient
                    ),
                ...
                if (
                    ...
                    if (error.message.code === -32098) {
                        ...
                        title =
                            error.message.data.message || this.env._t("Server Error");
                        body =
        ...
        async _loadDemoData() {
            const {confirmed} = await this.showPopup("ConfirmPopup", {
                ...
                body: this.env._t("Would you like to load demo data?"),
            });
            if (confirmed) {
                await this.rpc({
    ...
    ChromeKiosk.template = "ChromeKiosk";

    Registries.Component.add(ChromeKiosk);
