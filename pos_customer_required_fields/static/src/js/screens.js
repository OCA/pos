...
odoo.define('pos_order_zipcode.screens', function (require) {
    ...
    screens.PaymentScreenWidget.include({
        missing_customer_fields: function(){
            ...
            return this.pos.config.res_partner_required_fields_names.split(",").filter(
                function(name){
                    if (!customer[name]){
                        return true;
                    }
        ...
        order_is_valid: function(force_validation) {
            ...
            if (missing_fields.length > 0) {
                this.gui.show_popup('error', {
                    ...
                    'body': _t('Some data on the customer you picked are missing. Use the customer screen to edit: ') + missing_fields,
                });
                return false;
            }
    ...
    screens.ClientListScreenWidget.include({
        display_client_details: function(visibility,partner,clickpos){
            ...
            if (visibility==='edit'){
                const required_fields = this.pos.config.res_partner_required_fields_names.split(",");
