...
class PWA(Controller):
    ...
    def _get_pwa_manifest_icons(self, pwa_icon):
        ...
        if not pwa_icon:
            for size in [
                ...
                icons.append(
                    {
                        "src": "/pos_pwa_oca/static/img/icons/icon-%sx%s.png"
    ...
    def _get_pwa_manifest(self):
        ...
        pwa_icon = (
            request.env["ir.attachment"]
            .sudo()
            .search([("url", "like", "/pos_pwa_oca/icon.")])
        )
    ...
    @route("/pos_pwa_oca/manifest.webmanifest", type="http", auth="public")
